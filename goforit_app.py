...
with tab2:
    if not df_empresa.empty:
        st.subheader("Análisis Visual de Madurez")
        ...
        df_radar = df_empresa[(df_empresa['Foco'].isin(focos_sel)) & (df_empresa['Nombre'].isin(nombres_sel))]
        ...
        if not df_radar.empty:
            ...
            fig_radar.add_trace(go.Scatterpolar(
                ...
                name='Meta', line_color='#1E3A8A', fillcolor='rgba(30, 58, 138, 0.2)'
            ))
            fig_radar.add_trace(go.Scatterpolar(
                r=df_plot['Actual'], theta=df_plot['Dimensión'], fill='toself', 
                name='Hoy', line_color='#E11D48', fillcolor='rgba(225, 29, 72, 0.4)',
                marker=dict(size=10, line=dict(color="white", width=2))
            ))
            fig_radar.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 10])), height=550, margin=dict(l=80, r=80, t=20, b=20))
            st.plotly_chart(fig_radar, use_container_width=True)

            # Tabla Resumen con Semáforo
            st.markdown("### 📊 Indicadores por Pilar")
            def color_brecha(val):
                color = '#fecaca' if val > 2 else '#fef08a' if val > 0.5 else '#bbf7d0'
                return f'background-color: {color}'
            
            st.table(df_plot.style.applymap(color_brecha, subset=['Brecha']).format("{:.1f}"))
...
with tab3:
    if not df_empresa.empty:
        ...
        with col_btn2:
            ...
            st.download_button("📄 Reporte Word", data=word_bin, file_name=f"Informe_{empresa_sel}.docx")
