...
def leer_datos():
    try:
        ...
        mapeo = {"Distintos": "Capacidades_Distintivas"}
...
def generar_word(empresa, df_f, logo_p):
    ...
    for _, r in df_f.iterrows():
        ...
        doc.add_paragraph(f"Capacidades: {r['Capacidades_Distintivas']}")
    target = BytesIO()
    doc.save(target)
...
with st.sidebar:
    ...
    if emp_sel == "-- Nuevo Proyecto --":
        ...
        if st.button("🚀 Activar"):
            if n_emp:
                if enviar_datos({"Empresa": n_emp, "Dimensión": "Alineación", "Nombre": "Admin", "Actual": 1, "Objetivo": 1}):
...
with t1:
    ...
    with st.form("form_smart"):
        st.subheader("✏️ Datos de Evaluación" if row is not None else "➕ Nuevo Registro")
        ...
        f_fac = tx1.text_area("¿Qué facilita?", value=str(row['Facilita']) if row is not None else "")
        f_dif = tx1.text_area("¿Qué dificulta?", value=str(row['Dificulta']) if row is not None else "")
        f_no = tx2.text_area("¿Qué NO hacemos?", value=str(row['No_Hacemos']) if row is not None else "")
        f_cap = tx2.text_area("Capacidades Distintivas:", value=str(row['Capacidades_Distintivas']) if row is not None else "")
        
        a1 = st.text_input("Acción 1:", value=str(row['Accion_1']) if row is not None else "")
        a2 = st.text_input("Acción 2:", value=str(row['Accion_2']) if row is not None else "")
...
# --- FUNCIÓN DE FILTROS PARA TABS 2 Y 3 ---
...
with t2:
    if not df_emp.empty:
        ...
        if not df_f_rad.empty:
            ...
            m1.metric("Madurez", f"{df_f_rad['Actual'].mean():.1f}")
            m2.metric("Meta", f"{df_f_rad['Objetivo'].mean():.1f}")
            m3.metric("Gap", f"{(df_f_rad['Objetivo'].mean()-df_f_rad['Actual'].mean()):.1f}")
...
with t3:
    if not df_emp.empty:
        ...
        c_t.subheader("Resumen Ejecutivo de Hallazgos")
        with c_b:
