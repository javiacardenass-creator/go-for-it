...
# --- ESTADOS DE SESIÓN (Limpieza de Formulario) ---
...
# --- TAB 1: EVALUACIÓN (CON SELECTORES INTELIGENTES) ---
with tab1:
    # Listas dinámicas basadas en lo que ya existe en la base de datos para esta empresa
    ...
    with st.form(key=f"eval_form_{st.session_state.form_reset}"):
        ...
        # Selector de Foco (Existente vs Nuevo)
        modo_foco = c2.radio("¿Foco / Área?", ["Existente", "Nuevo"], horizontal=True)
        ...
        # Selector de Participante (Existente vs Nuevo)
        ca, cb, cc = st.columns([2, 1, 1])
        modo_nom = ca.radio("¿Participante?", ["Existente", "Nuevo"], horizontal=True)
        ...
        if st.form_submit_button("💾 Guardar y Sincronizar"):
            if nom and foc:
                ...
                if guardar_fila_google(datos):
                    ...
                    st.success(f"Información guardada.")
                    st.rerun()
                else: st.error("Error al conectar con Google Script.")

# --- TAB 2: RADAR & TABLA RESUMEN ---
fig_radar = None
with tab2:
    if not df_empresa.empty:
        ...
        with col_filt1:
            focos_sel = st.multiselect("Focos:", sorted(df_empresa['Foco'].unique()), default=sorted(df_empresa['Foco'].unique()))
        with col_filt2:
            nombres_sel = st.multiselect("Participantes:", sorted(df_empresa['Nombre'].unique()), default=sorted(df_empresa['Nombre'].unique()))
        ...
        if not df_radar_filtered.empty:
            ...
            fig_radar = go.Figure()
            fig_radar.add_trace(go.Scatterpolar(r=df_plot['Objetivo'], theta=df_plot['Dimensión'], fill='toself', name='Meta', line_color='#1E3A8A', fillcolor='rgba(30, 58, 138, 0.2)'))
            fig_radar.add_trace(go.Scatterpolar(r=df_plot['Actual'], theta=df_plot['Dimensión'], fill='toself', name='Hoy', line_color='#E11D48', fillcolor='rgba(225, 29, 72, 0.4)', marker=dict(size=10, line=dict(color="white", width=2))))
            fig_radar.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 10], gridcolor="#cbd5e1")), height=550, margin=dict(l=80, r=80, t=30, b=30), paper_bgcolor='rgba(0,0,0,0)')
            ...
            st.table(df_plot.style.applymap(estilo_brecha, subset=['Brecha']).format("{:.1f}"))
        else: st.warning("Aplica filtros para ver datos.")
