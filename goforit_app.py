...
# Configurado para usar Service Account desde Secrets
conn = st.connection("gsheets", type=GSheetsConnection)

def leer_datos():
    try:
        # ttl=0 para asegurar que siempre traiga lo último de INNOVIA, APS, etc.
...
def exportar_reporte_word(empresa, df, fig, logo_path):
    ...
    except: doc.add_paragraph("[Gráfico no disponible]")

    doc.add_heading('2. Detalle de Compromisos', level=1)
...
if not df_global.empty:
    df_global = df_global.dropna(subset=['Empresa'])
empresas_existentes = sorted(df_global['Empresa'].unique().tolist()) if not df_global.empty else []
    ...
    with st.sidebar.expander("✨ Registrar Nueva Empresa", expanded=True):
        ...
        if st.button("Activar Proyecto"):
            if nueva:
                ...
                if nuevo_logo: 
                    Image.open(nuevo_logo).save(os.path.join(LOGO_PATH, f"{nueva}.png"))
                st.success("Empresa registrada. Selecciónela en la lista.")
                st.rerun()
    st.stop()

# --- ZONA DE BORRADO ---
st.sidebar.divider()
with st.sidebar.expander("⚠️ Borrar Empresa"):
    if st.button(f"Confirmar eliminación definitiva"):
        df_final = df_global[df_global['Empresa'] != empresa_sel]
        conn.update(data=df_final)
        st.warning("Datos eliminados de la nube.")
        st.rerun()
...
with tab3:
    if not df_empresa.empty:
        c1, c2 = st.columns(2)
        p_sel = c1.selectbox("Pilar:", ["Todos"] + dimensiones)
        f_sel = c2.selectbox("Foco:", ["Todos"] + sorted(df_empresa['Foco'].unique().tolist()))
        df_f = df_empresa.copy()
        if p_sel != "Todos": df_f = df_f[df_f['Dimensión'] == p_sel]
        if f_sel != "Todos": df_f = df_f[df_f['Foco'] == f_sel]
        
        st.divider()
        word_data = exportar_reporte_word(empresa_sel, df_empresa, fig if 'fig' in locals() else None, logo_file)
        st.download_button("📄 Reporte Word", data=word_data, file_name=f"Informe_{empresa_sel}.docx")
        st.divider()
