...
# --- FUNCIÓN DE REPORTE CON ORDEN SOLICITADO ---
def generar_word_pro(empresa, df_f, fig_radar, logo_p):
    ...
    # 2. LA TABLA (Métricas)
    doc.add_heading('2. Calificaciones y Brechas (GAP)', level=1)
    t_q = doc.add_table(rows=1, cols=4); t_q.style = 'Light Grid Accent 1'
    for i, txt in enumerate(['Pilar', 'Hoy', 'Meta', 'GAP']): t_q.rows[0].cells[i].text = txt
    ...
    # 3. CAPACIDADES DISTINTIVAS (Mención especial)
    doc.add_heading('3. Capacidades Distintivas Identificadas', level=1)
    doc.add_paragraph("Factores críticos de éxito y ventajas competitivas detectadas durante la intervención:")
    caps = [str(x) for x in df_f['Capacidades_Distintivas'].unique() if str(x).strip() and str(x).lower() != "nan"]
    if caps:
        for cap in caps:
            p = doc.add_paragraph(style='List Bullet')
            p.add_run(cap).bold = True
    else:
        doc.add_paragraph("No se registraron capacidades específicas en este periodo.")

    # 4. LA MATRIZ (Detalle Cualitativo)
    doc.add_heading('4. Matriz de Diagnóstico Integral', level=1)
...
with st.sidebar:
    ...
    if emp_sel == "-- Nuevo Proyecto --":
        ...
        if u_logo_ex and st.button("💾 Guardar"):
...
t1, t2, t3 = st.tabs(["📝 Registro", "📊 Radar Analítico", "📋 Reporte Ejecutivo"])

# TAB 1: REGISTRO (SIN CAMBIOS)
with t1:
    st.subheader("Ingreso de Hallazgos")
    c1, c2, c3 = st.columns(3)
    p_master = ["Intimidad Cliente-Mercado", "Liderazgo Producto-Servicio", "Excelencia Operacional", "Flujo de Caja Sostenible", "Aprendizaje Constante", "Alineación Estratégica"]
    dim_f = c1.selectbox("Pilar:", p_master)
    foc_f = c2.text_input("Foco:") if c2.toggle("Nuevo Foco") else c2.selectbox("Foco:", sorted(df_emp['Foco'].unique()) if not df_emp.empty else ["General"])
    nom_f = c3.text_input("Nombre:") if c3.toggle("Nuevo Participante") else c3.selectbox("Participante:", sorted(df_emp['Nombre'].unique()) if not df_emp.empty else ["Admin"])
    match = df_emp[(df_emp['Dimensión'] == dim_f) & (df_emp['Foco'] == foc_f) & (df_emp['Nombre'] == nom_f)]
    row = match.iloc[0] if not match.empty else None
    with st.form("f_reg"):
        ca, cb = st.columns(2)
        v_act, v_obj = ca.slider("Hoy", 1, 10, int(row['Actual']) if row is not None else 5), cb.slider("Meta", 1, 10, int(row['Objetivo']) if row is not None else 8)
        tx1, tx2 = st.columns(2)
        def gv(r, c): return str(r[c]) if r is not None and c in r and pd.notna(r[c]) else ""
        f_fac, f_dif = tx1.text_area("Facilita", value=gv(row, 'Facilita')), tx1.text_area("Dificulta", value=gv(row, 'Dificulta'))
        f_no, f_cap = tx2.text_area("No hacemos", value=gv(row, 'No_Hacemos')), tx2.text_area("Capacidades", value=gv(row, 'Capacidades_Distintivas'))
...
# TAB 2: RADAR DINÁMICO + TABLA
with t2:
    if not df_emp.empty:
        c_f1, c_f2 = st.columns(2)
        p_filt = c_f1.multiselect("Pilares:", p_master, default=p_master)
        n_filt = c_f2.multiselect("Participantes:", df_emp['Nombre'].unique(), default=df_emp['Nombre'].unique())
        ...
        if not df_p.empty:
            ...
            df_m = df_p.copy()
            df_m['GAP'] = (df_m['Objetivo'] - df_m['Actual']).map('{:.1f}'.format)
            df_m['Actual'] = df_m['Actual'].map('{:.1f}'.format)
            df_m['Objetivo'] = df_m['Objetivo'].map('{:.1f}'.format)
            st.table(df_m)
