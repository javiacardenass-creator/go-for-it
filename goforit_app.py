...
def leer_datos():
    try:
        return conn.read(ttl="0m") # Leer datos frescos
    except:
...
if empresa_sel == "-- Nuevo Proyecto --":
    with st.sidebar.expander("✨ Registrar Nueva Empresa", expanded=True):
        ...
        nuevo_logo = st.file_uploader("Cargar Logo Inicial", type=["png", "jpg"])
        if st.button("Activar Proyecto"):
            if nueva:
                # El registro se crea al guardar la primera evaluación
                if nuevo_logo: Image.open(nuevo_logo).save(os.path.join(LOGO_PATH, f"{nueva}.png"))
                st.success(f"Proyecto {nueva} listo. Inicie la evaluación.")
    st.stop()
...
with tab1:
    ...
    nombres_reg = sorted(df_empresa[df_empresa['Dimensión'] == dim_actual]['Nombre'].unique().tolist()) if not df_empresa.empty else []
...
with tab3:
    if not df_empresa.empty:
        ...
        p_s = col_f1.selectbox("Pilar:", ["Todos"] + dimensiones)
        f_s = col_f2.selectbox("Foco:", ["Todos"] + sorted(df_empresa['Foco'].unique().tolist()))
        ...
        st.download_button("📄 Descargar Reporte (Word)", data=word_data, file_name=f"Informe_{empresa_sel}.docx")
        st.divider()
        
        tabla_p = df_f.pivot_table(index='Nombre', columns='Dimensión', values='Actual', aggfunc='mean').fillna(0)
        st.dataframe(tabla_p.style.background_gradient(cmap='Blues'), use_container_width=True)
