...
with tab1:
    ...
    with col_modos_1:
        modo_foco = st.radio("¿Foco?", ["Existente", "Nuevo"], horizontal=True, key=f"mf_{st.session_state.form_reset}")
    with col_modos_2:
        modo_nom = st.radio("¿Participante?", ["Existente", "Nuevo"], horizontal=True, key=f"mn_{st.session_state.form_reset}")
...
with tab2:
    if not df_empresa.empty:
        st.subheader("Análisis de Madurez")
        col_f1, col_f2 = st.columns(2)
        focos_sel = col_f1.multiselect("Focos:", sorted(df_empresa['Foco'].unique()), default=sorted(df_empresa['Foco'].unique()))
        nombres_sel = col_f2.multiselect("Participantes:", sorted(df_empresa['Nombre'].unique()), default=sorted(df_empresa['Nombre'].unique()))
        ...
        if not df_radar_filtered.empty:
            ...
            fig_radar = go.Figure()
            fig_radar.add_trace(go.Scatterpolar(r=df_plot['Objetivo'], theta=df_plot['Dimensión'], fill='toself', name='Meta'))
            fig_radar.add_trace(go.Scatterpolar(r=df_plot['Actual'], theta=df_plot['Dimensión'], fill='toself', name='Hoy'))
...
# --- TAB 3: RESUMEN EJECUTIVO (EL BOTÓN ESTÁ AQUÍ) ---
with tab3:
    if not df_empresa.empty:
        st.subheader("Plan de Acción y Reporte")
        col_inf, col_btn = st.columns([3, 1])
        
        with col_btn:
            # Generar el binario del Word
        ...
        st.info("Aún no hay datos registrados para esta empresa.")