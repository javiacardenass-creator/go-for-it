...
def leer_datos():
    try:
        ...
        # Normalización de columnas para evitar pérdida de datos
...
def generar_word_pro(empresa, df_f, fig_radar, logo_p):
    ...
    for d in df_f['Dimensión'].unique():
        ...
        r = t_q.add_row().cells
        r[0].text, r[1].text = d, f"{sub['Actual'].mean():.1f}"
        r[2].text, r[3].text = f"{sub['Objetivo'].mean():.1f}", f"{(sub['Objetivo'].mean()-sub['Actual'].mean()):.1f}"

    # 3. Matriz Cualitativa
    ...
    # 4. Capacidades
    doc.add_heading('4. Capacidades Distintivas Identificadas', level=1)
...
with st.sidebar:
    ...
    if emp_sel == "-- Nuevo Proyecto --":
        ...
        if u_logo_ex and st.button("💾 Guardar Logo"):
            ...
            st.success("Logo actualizado correctamente."); st.rerun()

# --- INTERFAZ PRINCIPAL ---
df_emp = df_global[df_global['Empresa'] == emp_sel]
logo_file = os.path.join(LOGO_PATH, f"{emp_sel}.png")

t1, t2, t3 = st.tabs(["📝 Registro y Edición", "📊 Radar Dinámico", "📋 Reporte Ejecutivo"])

# --- TAB 1: REGISTRO ---
with t1:
    st.subheader("Ingreso de Hallazgos")
    c1, c2, c3 = st.columns(3)
    pilares_master = ["Intimidad Cliente-Mercado", "Liderazgo Producto-Servicio", "Excelencia Operacional", "Flujo de Caja Sostenible", "Aprendizaje Constante", "Alineación Estratégica"]
    dim_f = c1.selectbox("Pilar:", pilares_master)
    ...
    with st.form("form_registro"):
        ...
        if st.form_submit_button("💾 Guardar y Sincronizar"):
            payload = {"Empresa": emp_sel, "Dimensión": dim_f, "Foco": foc_f, "Nombre": nom_f, "Actual": v_act, "Objetivo": v_obj, "Brecha": v_obj-v_act, "Facilita": f_fac, "Dificulta": f_dif, "No_Hacemos": f_no, "Capacidades_Distintivas": f_cap, "Accion_1": a1, "Accion_2": a2}
            if enviar_datos(payload):
                st.cache_data.clear(); st.rerun()

# --- TAB 2: RADAR DINÁMICO ---
with t2:
    if not df_emp.empty:
        col_f1, col_f2 = st.columns(2)
        p_filt = col_f1.multiselect("Filtrar Pilares en Gráfico:", pilares_master, default=pilares_master)
...
with t3:
    if not df_emp.empty:
        ...
        if 'fig_actual' in st.session_state:
            ...
            st.download_button("📥 Descargar Reporte Integral (Word)", data=w_bin, file_name=f"Informe_{emp_sel}.docx")
        else: st.warning("⚠️ Entra primero a la pestaña Radar para procesar la imagen.")
        st.dataframe(df_emp)